...
$co=mysql_query("select direccion.correo from direccion where correo='$correo'",$link);
...
if(empty($nombre)||empty($apellido)||empty($correo)||empty($telefono)||empty($cargo)||empty($clave))
	...
	$men='debe llenar todos los datos';	
		header('Location:actualizar_personal_dir.php?men='.urlencode($men).'&id='.urlencode($id));
	}
...
$query= "UPDATE direccion SET nombre='$nombre',
								apellido='$apellido',
								correo='$correo',
								skipe='$skipe',
								telefono='$telefono',
