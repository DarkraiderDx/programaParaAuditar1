...
$co=mysql_query("select reg_proyecto.correo from reg_proyecto where correo='$correo'",$link);
...
if(empty($nombre)||empty($apellido)||empty($correo)||empty($telefono)||empty($cargo)||empty($clave))
	...
	$men='debe llenar todos los datos';	
		header('Location:actualizar_personal_pro.php?men='.urlencode($men).'&id='.urlencode($id));
	}
...
$query= "UPDATE reg_proyecto SET nombre='$nombre',
								apellido='$apellido',
								correo='$correo',
								skipe='$skipe',								
								telefono='$telefono',
